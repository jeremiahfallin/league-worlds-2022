...
export default function ChampionStats({ results, leagues, league }) {
  ...
  return (
    <div>
      <Head>
        <title>{`${league} 2022 Champion Data`}</title>
...
export async function getStaticPaths() {
  ...
  const leagues = allLeagues.filter((league) => {
    return (
      league.startsWith("LCS/") ||
      league.startsWith("LEC/") ||
      league.startsWith("LCK/") ||
      league.startsWith("LPL/")
    );
  });
